...
export class FrequentComponent implements OnInit {
  ...
  filteredPatterns: FrequentPattern[] = [];
  paginatedPatterns: FrequentPattern[] = [];
  selectedPattern: FrequentPattern | null = null;
  ...
  loadPatternsFromJson(): void {
    this.isLoading = true;
    this.http.get<FrequentPattern[]>('assets/data/frequent.json')
      .subscribe({
        next: (data) => {
          this.frequentPatterns = data;
          this.filteredPatterns = [...this.frequentPatterns];
        ...
        error: (err) => {
          ...
          this.error = 'Failed to load frequent questions';
          this.isLoading = false;
        }
      });
  }
  ...
  extractCategories(): void {
    const categorySet = new Set(this.frequentPatterns.map(p => p.category));
  ...
  selectPattern(pattern: FrequentPattern): void {
  ...
  loadMarkdownFile(filename: string): void {
    this.isLoading = true;
    const filePath = `answers/frequent/${filename}`;
  ...
  filterPatterns(): void {
    this.filteredPatterns = this.frequentPatterns.filter(pattern => {
