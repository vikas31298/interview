...
@Component({
  ...
  styleUrl: './behavioral.component.scss'
})
export class BehavioralComponent implements OnInit {
  behavioralPatterns: BehavioralPattern[] = [];
  filteredPatterns: BehavioralPattern[] = [];
  paginatedPatterns: BehavioralPattern[] = [];
  selectedPattern: BehavioralPattern | null = null;
  ...
  loadPatternsFromJson(): void {
    this.isLoading = true;
    this.http.get<BehavioralPattern[]>('assets/data/behavioral.json')
      .subscribe({
        next: (data) => {
          this.behavioralPatterns = data;
          this.filteredPatterns = [...this.behavioralPatterns];
        ...
        error: (err) => {
          ...
          this.error = 'Failed to load behavioral patterns';
          this.isLoading = false;
        }
      });
  }
  ...
  extractCategories(): void {
    const categorySet = new Set(this.behavioralPatterns.map(p => p.category));
  ...
  selectPattern(pattern: BehavioralPattern): void {
  ...
  loadMarkdownFile(filename: string): void {
    this.isLoading = true;
    const filePath = `answers/behavioral/${filename}`;
  ...
  filterPatterns(): void {
    this.filteredPatterns = this.behavioralPatterns.filter(pattern => {
