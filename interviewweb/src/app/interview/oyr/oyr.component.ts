...
@Component({
  selector: 'app-oyr',
  standalone: false,
  templateUrl: './oyr.component.html',
  styleUrl: './oyr.component.scss'
})
export class OyrComponent implements OnInit {
  oyrPatterns: OyrPattern[] = [];
  filteredPatterns: OyrPattern[] = [];
  paginatedPatterns: OyrPattern[] = [];
  selectedPattern: OyrPattern | null = null;
  ...
  loadPatternsFromJson(): void {
    this.isLoading = true;
    this.http.get<OyrPattern[]>('assets/data/oyr.json')
      .subscribe({
        next: (data) => {
          this.oyrPatterns = data;
          this.filteredPatterns = [...this.oyrPatterns];
        ...
        error: (err) => {
          ...
          this.error = 'Failed to load OYR patterns';
          this.isLoading = false;
        }
      });
  }
  ...
  extractCategories(): void {
    const categorySet = new Set(this.oyrPatterns.map(p => p.category));
  ...
  selectPattern(pattern: OyrPattern): void {
  ...
  loadMarkdownFile(filename: string): void {
    this.isLoading = true;
    const filePath = `answers/oyr/${filename}`;
  ...
  filterPatterns(): void {
    this.filteredPatterns = this.oyrPatterns.filter(pattern => {
